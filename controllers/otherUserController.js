...
const getOtherUserController = async (req, res, next) => {
  try {
...
const userSubscriptionsController = async (req, res, next) => {
  try {
    ...
    if (subscribedUsers.length === 0) {
      ...
      const paginatedSubscribedUsers = subscribedUsers.slice(
        skip,
        skip + limit
      );
...
const userDeleteSubscriptionsController = async (req, res, next) => {
  try {
    ...
    subscriptions = subscriptions.filter(
      (subscriptionId) =>
        subscriptionId.toString() !== unSubscribeUser.toString()
    );
